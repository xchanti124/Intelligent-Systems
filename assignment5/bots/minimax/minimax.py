...
class MiniMaxBot(Bot):
    ...
    def get_move(self, perspective: PlayerPerspective, leader_move: Optional[Move]) -> Move:
        assert (perspective.get_phase(
        ) == GamePhase.TWO), "MiniMaxBot can only work in the second phase of the game."
        _, move = self.value(
        ...
        for move in valid_moves:
            ...
            if leader_move is None:
                ...
                new_game_state = engine.play_one_trick(
                    game_state=state, new_leader=leader, new_follower=follower)
                ...
                if winning_info:
                    ...
                    value, _ = self.value(
                        new_game_state, engine, None, next_maximizing)
